...
def shrinkPortNameForChiselBlockBox():
    ...
    AxiLiteInput = input('AXI lite port inside (y/n): ')
    AxiLiteMode = (AxiLiteInput=='y') or (AxiLiteInput=='Y')
    if AxiLiteMode:
        AxiLiteSelectSM = input('Slave or Master (s/m): ')
        if (AxiLiteSelectSM=='s') or (AxiLiteSelectSM=='S'):
            AxiLiteSelectSM = 's_'
        else:
            AxiLiteSelectSM = 'm_'
    ...
    with open(fileName) as f:
        for line in f:
            ...
            if AxiLiteMode and temp[1][0:4] == 'axi_':
                temp[1] = AxiLiteSelectSM+temp[1]
